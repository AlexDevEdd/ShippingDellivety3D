    ...
    public class InspectorEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            foreach (var method in this.methods)
            {
                ...
                if (methodDrawer.GetType() == typeof(ButtonShowIfMethodDrawer))
                {
                    ButtonShowIfMethodDrawer buttonShowIfMethodDrawer = (ButtonShowIfMethodDrawer)methodDrawer;
                    if (buttonShowIfMethodDrawer.CanDrawProperty(this.target, method))
                    {
                        if (string.IsNullOrEmpty(conditionName))
                        {
                            conditionName = buttonShowIfMethodDrawer.GetButtonShowAttribute(method).Conditions[0];
                            GUILayout.Space(20);
                            GUILayout.BeginHorizontal();
                        }
                        else if (((ButtonShowIfMethodDrawer)methodDrawer).GetButtonShowAttribute(method).Conditions[0] != conditionName)
                        {
                            conditionName = buttonShowIfMethodDrawer.GetButtonShowAttribute(method).Conditions[0];
            ...
            foreach (var method in this.methods)
            {
                DrawerAttribute drawerAttribute = (DrawerAttribute)method.GetCustomAttributes(typeof(DrawerAttribute), true)[0];
                MethodDrawer methodDrawer = MethodDrawerDatabase.GetDrawerForAttribute(drawerAttribute.GetType());
                if (methodDrawer != null && methodDrawer.GetType() != typeof(ButtonShowIfMethodDrawer))
                {
                    methodDrawer.DrawMethod(this.target, method);
                }
            }
